...
if __name__ == "__main__":
    ...
    # Step 2: (removed) DRL/trained-policy approach omitted.
    dprint("\n[STEP 2] Skipping DRL/trained-policy loading — comparing heuristics only")

    # Step 3: Create baseline solvers (FIFO + SPT heuristic)
    dprint("\n[STEP 3] Creating baseline solvers (FIFO + SPT)...")
    ...
    # Step 5: Create PolicyConformance analyzer comparing FIFO vs SPT heuristics
    dprint("\n[STEP 5] Initializing PolicyConformance analyzer (FIFO vs SPT)...")
    ...
    dprint(f"[INFO] Running {num_episodes} episodes per heuristic (FIFO and SPT)...")

    try:
            # Frequency runs for FIFO and SPT heuristics
            fifo_frequencies = {}
            for episode in range(num_episodes):
                dprint(f"  Episode {episode + 1}/{num_episodes} (FIFO)...")
                freq = conformance.frequency_run(fifo_solver, simulation_length)
                for state, count in freq.items():
                    fifo_frequencies[state] = fifo_frequencies.get(state, 0) + count

            dprint(f"[SUCCESS] FIFO policy visited {len(fifo_frequencies)} unique states")
            # Show top-10 FIFO visited states for diagnostics
            top_fifo = sorted(fifo_frequencies.items(), key=lambda x: x[1], reverse=True)[:10]
            dprint("[DIAG] Top FIFO visited states (state_tuple -> count):")
            for st, cnt in top_fifo:
                dprint(f"  {st} -> {cnt}")

            spt_frequencies = {}
            for episode in range(num_episodes):
                dprint(f"  Episode {episode + 1}/{num_episodes} (SPT)...")
            ...
            # Show top-10 SPT visited states for diagnostics
            top_spt = sorted(spt_frequencies.items(), key=lambda x: x[1], reverse=True)[:10]
            dprint("[DIAG] Top SPT visited states (state_tuple -> count):")
            for st, cnt in top_spt:
                dprint(f"  {st} -> {cnt}")

            # Frequency runs for RANDOM heuristic
            random_frequencies = {}
            for episode in range(num_episodes):
                dprint(f"  Episode {episode + 1}/{num_episodes} (RANDOM)...")
            ...
            top_rand = sorted(random_frequencies.items(), key=lambda x: x[1], reverse=True)[:10]
            dprint("[DIAG] Top RANDOM visited states (state_tuple -> count):")
            for st, cnt in top_rand:
                dprint(f"  {st} -> {cnt}")

            # Calculate state overlap (FIFO vs SPT vs RANDOM)
            all_states = set(fifo_frequencies.keys()) | set(spt_frequencies.keys()) | set(random_frequencies.keys())
            overlapping_states = set(fifo_frequencies.keys()) & set(spt_frequencies.keys())
            # Also report how much FIFO and SPT overlap with RANDOM for diagnostics
            overlap_ratio = len(overlapping_states) / len(all_states) if all_states else 0
            fifo_rand_overlap = len(set(fifo_frequencies.keys()) & set(random_frequencies.keys()))
            spt_rand_overlap = len(set(spt_frequencies.keys()) & set(random_frequencies.keys()))

            dprint(f"\n[ANALYSIS] State Visit Frequency:")
            dprint(f"  - FIFO policy visited states: {len(fifo_frequencies)}")
            ...
            dprint(f"  - FIFO vs RANDOM overlap: {fifo_rand_overlap} states")
            dprint(f"  - SPT vs RANDOM overlap: {spt_rand_overlap} states")

            # Diagnostic: show some exclusive states to illustrate low overlap
            fifo_states = set(fifo_frequencies.keys())
            spt_states = set(spt_frequencies.keys())
            exclusive_fifo = list(fifo_states - spt_states)[:5]
            exclusive_spt = list(spt_states - fifo_states)[:5]
            dprint(f"[DIAG] Example exclusive FIFO states (up to 5): {exclusive_fifo}")
            dprint(f"[DIAG] Example exclusive SPT states (up to 5): {exclusive_spt}")
    ...
    try:
        # Build two PolicyConformance analyzers to record actions both ways
        conformance_spt_fifo = PolicyConformance(
            gym_problem=problem,
            heuristic_solver=fifo_solver,
            gym_solver=spt_solver,
            state_variables=state_variables,
        ...
        conformance_fifo_spt = PolicyConformance(
            gym_problem=problem,
            heuristic_solver=fifo_solver,
            gym_solver=spt_solver,
            state_variables=state_variables,
        ...
        # Run action episodes: first SPT leads, FIFO observes; then FIFO leads, SPT observes
        for episode in range(num_episodes):
            dprint(f"  Episode {episode + 1}/{num_episodes} (SPT leads, FIFO observes)...")
            conformance_spt_fifo.action_run(spt_solver, fifo_solver, length=simulation_length)
        for episode in range(num_episodes):
            dprint(f"  Episode {episode + 1}/{num_episodes} (FIFO leads, SPT observes)...")
            conformance_fifo_spt.action_run(fifo_solver, spt_solver, length=simulation_length)

        # Collect mappings
        p1_states_s = set(conformance_spt_fifo.p1_state_action_mapping.keys())
        p2_states_s = set(conformance_spt_fifo.p2_state_action_mapping.keys())
        p1_states_f = set(conformance_fifo_spt.p1_state_action_mapping.keys())
        p2_states_f = set(conformance_fifo_spt.p2_state_action_mapping.keys())

        dprint(f"[SUCCESS] Collected action mappings from both directions")
        dprint(f"  - SPT-led action states: {len(p1_states_s)}; FIFO-observed states: {len(p2_states_s)}")
        dprint(f"  - FIFO-led action states: {len(p1_states_f)}; SPT-observed states: {len(p2_states_f)}")

        # Diagnostic: Print action diversity using the two separate mappings
        dprint(f"\n[DEBUG] Action diversity analysis:")
        mapping_spt = conformance_spt_fifo.p1_state_action_mapping  # actions taken by SPT when SPT led
        mapping_fifo = conformance_fifo_spt.p1_state_action_mapping  # actions taken by FIFO when FIFO led

        p1_states = set(mapping_spt.keys())
        p2_states = set(mapping_fifo.keys())
        ...
        # Most common actions across observed states
        ...
        dprint(f"  - Most common SPT actions: {top_actions(mapping_spt, p1_states)}")
        dprint(f"  - Most common FIFO actions: {top_actions(mapping_fifo, p2_states)}")

        # Compute action agreement by querying both solvers on the SAME snapshot for each state.
        # This avoids the bias introduced by recording actions under different leading policies.
        action_agreement = 0
        total_actions = 0
        ...
        for state in common_states:
            # pick a representative snapshot from either conformance object
            snapshot = None
            if state in conformance_spt_fifo.states_in_observation and conformance_spt_fifo.states_in_observation[state]:
                snapshot = conformance_spt_fifo.states_in_observation[state][0]
            elif state in conformance_fifo_spt.states_in_observation and conformance_fifo_spt.states_in_observation[state]:
                snapshot = conformance_fifo_spt.states_in_observation[state][0]
            if snapshot is None:
                continue

            act_spt = conformance_spt_fifo._get_solver_action(snapshot, spt_solver)
            act_fifo = conformance_spt_fifo._get_solver_action(snapshot, fifo_solver)

            # Normalize None to 'postpone' for comparison
            ...
            total_actions += 1
            # relaxed: compare only transition name portion (before '|')
            try:
        ...
        # Diagnostics: show a few disagreement examples
        if disagreement_examples:
            ...
            for ex in disagreement_examples:
                ...
                # Print full action count dicts for inspection
                dprint(f"    SPT actions: {mapping_spt.get(st)}")
                dprint(f"    FIFO actions: {mapping_fifo.get(st)}")
                # Query the solvers on an actual state snapshot (if available) to compare returned binding keys
                state_snap = None
                if st in conformance_spt_fifo.states_in_observation:
                    state_snap = conformance_spt_fifo.states_in_observation[st][0]
                elif st in conformance_fifo_spt.states_in_observation:
                    state_snap = conformance_fifo_spt.states_in_observation[st][0]
                if state_snap is not None:
                    act_spt = conformance_spt_fifo._get_solver_action(state_snap, spt_solver)
                    act_fifo = conformance_spt_fifo._get_solver_action(state_snap, fifo_solver)
                    dprint(f"    Queried on same snapshot: SPT -> {act_spt} ; FIFO -> {act_fifo}")

        # NEW DIAGNOSTIC: compute agreement across the UNION of observed states
        # (not just the intersection). This reveals whether solvers behave the
        # same on states seen by only one policy. We also compute a visit-weighted
        # agreement using the frequency counts collected earlier.
        try:
            union_states = set(conformance_spt_fifo.states_in_observation.keys()) | set(conformance_fifo_spt.states_in_observation.keys())
            union_total = 0
            union_agree = 0
            ...
            for st in union_states:
                # pick a snapshot from either mapping if available
                snap = None
                if st in conformance_spt_fifo.states_in_observation and conformance_spt_fifo.states_in_observation[st]:
                    snap = conformance_spt_fifo.states_in_observation[st][0]
                elif st in conformance_fifo_spt.states_in_observation and conformance_fifo_spt.states_in_observation[st]:
                    snap = conformance_fifo_spt.states_in_observation[st][0]
                if snap is None:
                    continue

                act_spt_q = conformance_spt_fifo._get_solver_action(snap, spt_solver)
                act_fifo_q = conformance_spt_fifo._get_solver_action(snap, fifo_solver)
                ...
                # visit-weight: how often this state was visited across the two samplings
                vcount = fifo_frequencies.get(st, 0) + spt_frequencies.get(st, 0)
        ...
        # (No separate 'trained vs SPT' comparison in heuristic-only mode)
    ...
    try:
        ...
        expected_return_p1, expected_return_p2, delta_expected_return = conformance.expected_reward_run(
            fifo_solver, spt_solver, rho=0.4, eta=0, num_rollouts=10, num_steps=100)
