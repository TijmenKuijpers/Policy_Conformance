...
def merge_estimates(*raw_dicts):
    """Average action probabilities for the same observation across multiple runs."""
    all_obs = set().union(*raw_dicts)
    merged_out = {}
    for obs in all_obs:
        merged = {}
        for src in raw_dicts:
            for a, p in src.get(obs, {}).items():
                merged.setdefault(a, []).append(p)
        merged_out[obs] = {a: sum(v) / len(v) for a, v in merged.items()}
    return merged_out

# π₁ appears in two runs (π₁ vs π₂  and  π₁ vs π₃)
# π₂ appears in two runs (π₁ vs π₂  and  π₂ vs π₃)
# π₃ appears in two runs (π₁ vs π₃  and  π₂ vs π₃)
raw_pi1 = merge_estimates(raw_pi1_12, raw_pi1_13)
raw_pi2 = merge_estimates(raw_pi2,    raw_pi2_23)
raw_pi3 = merge_estimates(raw_pi3,    raw_pi3_23)

pi1_pa = remap_probs(raw_pi1)
pi2_pa = remap_probs(raw_pi2)
pi3_pa = remap_probs(raw_pi3)
...
fig, axes = plt.subplots(1, 3, figsize=(15, max(6, len(all_obs) * 0.45 + 1)),
                          sharey=True)
